...
DEFAULT_KEY = bytes(b'\x01\x00\x01\x00')

DEFAULT_MOD = bytes(b'\xD5\xA3\xD6\xAB\x0F\x0D\xC5\x0F\xC3\xFA\x6E\x78\x9D\x0B\xE3\x32'
...
def decode_signature(bytes_sig, bytes_key=None, bytes_mod=None):
    ...
    key = DEFAULT_KEY if bytes_key is None else bytes_key
    mod = DEFAULT_MOD if bytes_mod is None else bytes_mod
